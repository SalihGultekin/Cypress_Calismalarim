...
declare global {
   namespace Cypress {
     ...
     checkTextOfTheElements(selector,text:string): Chainable<void>
     ...
     dynamitWait(selector: string): Chainable<void>
 ...
 Cypress.Commands.add('checkTextOfTheElements', (selector, text) =>{
    cy.get(selector)
    .should('be.visible')
    .and('have.text',text)
 })
